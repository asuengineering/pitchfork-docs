				...
				if (! empty($doc_categories)) {
					...
					foreach ($doc_categories as $doc_category) {
						...
						if ( get_queried_object_id() == $doc_category->term_id) {
							echo '<a class="nav-link is-active" href="' . get_term_link($doc_category) . '">' . $doc_category->name . '</a>';
						} else {
							echo '<a class="nav-link" href="' . get_term_link($doc_category) . '">' . $doc_category->name . '</a>';
						}
						echo '</div>';
					}
					echo '</nav>';
					...
					while ( have_posts() ) {
						...
						<article <?php post_class(); ?> id="post-<?php the_ID(); ?>">
							...
							<div class="entry-content">
								...
								<p class="modified"><?php printf( __( 'Last updated: %1$s at  %2$s', 'pitchfork' ),
									get_the_modified_date( 'F j, Y' ),
									get_the_modified_date( 'g:i a' )); ?></p>
